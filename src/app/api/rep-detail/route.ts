...
export async function GET() {
  ...
  // Cache the live member + sponsored/cosponsored fetch for 6h. Votes
  // come from the separately-scheduled 4h `rep_votes` cron payload and
  // are merged in at request time (they're already cached themselves).
  const cacheKey = `rep_${BIOGUIDE}_basics`;
  const basics = await lazyCached(cacheKey, TTL.HOURS_6, async () => {
    ...
    // Don't cache an empty result — likely a transient rate-limit hit.
    if (!member && !spJson && !csJson) return null;
  ...
  // Votes — separately cached by the 4h `rep_votes` cron. Always read
  // fresh-from-DB so a recent vote shows up before basics TTL expires.
  ...
  if (!basics) {
    return NextResponse.json({ error: 'Congress.gov returned empty for this member' }, { status: 502 });
  }
